...
@Test func testGTPPrintSGFEmptyBoard() async throws {
    ...
    // No moves played: the trailing semicolon-list should be absent.
...
@Test func testGTPPrintSGFEmitsHandicap() async throws {
    let katago = KataGoInference()
    let handler = GTPHandler(katago: katago)
    // Place two handicap stones; printsgf should record them as AB[...]
    // and indicate white to move via PL[W] (since black is the default).
    ...
    // Default for handicap is white-to-move; this matches the SGF default,
    // so PL[] is intentionally omitted.
...
@Test func testGTPLoadSGFBasic() async throws {
    ...
    // After loadsgf, the printed SGF should reflect the loaded position.
...
@Test func testSGFParserSkipsVariations() throws {
    // Main line: B[aa] → W[bb]. Variation B[cc] → W[dd] should be ignored.
